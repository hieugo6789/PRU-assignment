...
public class Inventory : MonoBehaviour, ISaveManager
{
    ...
    {
        // Kiểm tra xem có đủ vật liệu không trước khi thực hiện xóa
        foreach (var requiredMaterial in _requiredMaterials)
        {
            if (stashDictianory.TryGetValue(requiredMaterial.data, out InventoryItem stashValue))
            {
                if (stashValue.stackSize < requiredMaterial.stackSize)
                {
        ...
        // Loại bỏ vật liệu sau khi đã xác nhận có đủ
        foreach (var requiredMaterial in _requiredMaterials)
        {
            RemoveSpecificAmountOfItem(requiredMaterial.data, requiredMaterial.stackSize);
        }

        // Thêm item được craft vào inventory
    ...
    // Hàm mới để xóa một số lượng cụ thể của một item
    private void RemoveSpecificAmountOfItem(ItemData itemData, int amount)
    {
        if (stashDictianory.TryGetValue(itemData, out InventoryItem stashValue))
        {
            if (stashValue.stackSize > amount)
            {
                stashValue.stackSize -= amount;
            }
            else
            {
                stash.Remove(stashValue);
        ...
        UpdateSlotUI(); // Cập nhật UI sau khi đã thay đổi số lượng
    }
